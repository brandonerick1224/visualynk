...
visualynk = function(visualynk) {
        ...
        b.each(function(b) {
          if (b.children !== undefined) {
            ...
            if(b.children) {
                ...
                }).attr("src", "vendor/plugins/2d_model/img/category.png").style('padding-left', '15px')
                ...
                if($scope.user.crudpermissions.p_ng_d) {
                    b.append("span").attr("class", "taxonomy-group").attr("ng-click", function(b){
                        return "deleteNG($event, "+b.nodeId+", '"+b.label+"')";
                    }).append("i").attr("class", "icon-deleteng pull-right");
                }

                if($scope.user.crudpermissions.p_ng_u) {
                    b.append("span").attr("class", "taxonomy-group").attr("ng-click", function(b){
                        return "openNGUpdate($event, "+b.nodeId+", '"+b.label+"')";
                    }).append("i").attr("class", "icon-updateng pull-right");
                }

                if($scope.user.crudpermissions.p_ng_u || $scope.user.crudpermissions.p_ng_d) {
    ...
    a.taxonomy.onClick = function() {
        ...
        if (void 0 !== a.graph.getRootNode().count) {
            ...
            $(".vslynk-container-results").css("height" ,query_height + "px" );
    ...
    a.queryviewer.updateQuery = function() {
        ...
        if($scope.user.crudpermissions.p_ne_c) {
            a.queryviewer.queryConstraintSpanElements.append("span").attr("class", function(b){
                ...
                var nodeId = d.nodeId
                if(b.type === a.graph.node.NodeTypes.ROOT && b.term != "Visualynk" && nodeId>0)
